...
if(isset($_SESSION["user"])) {
    ...
    if($user->rowCount() == 1) {
        ...
        if($order->rowCount() == 1) {
            ...
            $bag = $db->query("select * from clientbets where orderid = ".$oid);
            if($bag->rowCount() > 0) {
                foreach($bag as $bag_bet) {
                    ...
                    echo '  <div class="edit" onclick="loadContent(\'bet.php?betid='.$bag_bet["betid"].'&edit=true\')"><div class="side0g">Edit</div><div class="arrow0g"></div></div>';
                    echo '  <div class="matchinfo">';
                    echo '      <div class="matchdetail">'.date('D, jS F Y',strtotime($bet["betcloses"])).'</div>';
                    echo '      '.$teams[0].' vs. '.$teams[1];
                    echo '      <div class="matchdetail">'.$category.'</div>';
